            ...
            <?php elseif ($task->getStatus() === TaskEntity::STATUS_MERGED): ?>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        } else if ($task->getParent()->private() && !$task->getParent()->own() && !Yii::$app->user->isManager($task->getProjectId())) {
                    ...
                    <div class="comment"
                        ...
                        <div class="media-left">
                            ...
                            if(!$comment->getDeleted()) {
                                echo Html::img($comment->getSender()->getAvatarAlias(), ['class' => 'comment-avatar']);
                            }
                            ?>
                        </div>
                        <div class="media-right">
                            <div class="comment-title">
                                <h5 class="comment-fio no-margin-top">
                                    ...
                                    if (!$comment->getDeleted()) {
                                        if (Yii::$app->user->isGuest) {
                                            echo Html::a($comment->getSender()->getUsername(true), ['/site/login'], ['name' => $comment->getId()]);
                                        } else {
                                            if ($comment->getSenderId() === Yii::$app->user->getId()) {
                                                echo Html::a($comment->getSender()->getUsername(true), ['/profile/my-projects'], ['name' => $comment->getId()]);
                                            } else {
                                                echo Html::a($comment->getSender()->getUsername(true), ['/profile/view', 'id' => $comment->getSender()->getId()], ['name' => $comment->getId()]);
                                            }
                                        }
                                    }
                                    ?>
                                    ...
                                    <div class="comment-parent">
                                        <?php if(!$parent->getDeleted() && !$parent->getPrivate()): ?>
                                            <div class="comment-parent-username">
                                                <?= Html::a($parent->getSender()->getUsername(true), ['/profile/view', 'id' => $parent->getSender()->getId()]) ?>
                                            </div>
